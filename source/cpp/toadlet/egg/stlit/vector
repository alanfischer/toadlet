...
#ifndef STLIT_VECTOR_H
#define STLIT_VECTOR_H
...
public:
    ...
    public:
		...
		typedef typename vector<Type>::reference reference;
		typedef typename vector<Type>::const_reference const_reference;
		typedef typename vector<Type>::pointer pointer;
		typedef typename vector<Type>::difference_type difference_type;
		typedef typename vector<Type>::iterator_category iterator_category;

		inline iterator():parent(NULL),index(0){}
		inline iterator(vector *p,int i):parent(p),index(i){}
    ...
    public:
		...
		inline const_iterator(vector *p,int i):parent(p),index(i){}
	...
	inline vector(int size,const Type &value):
		mSize(0),
		mCapacity(0),
		mData(NULL)
	{
	...
	template<typename Type2> inline vector(const vector<Type2> &c):
		mSize(0),
		mCapacity(0),
		mData(NULL)
	{
	...
	inline bool equals(const vector &c) const{
		if(mSize!=c.mSize){
			return false;
		}
	...
	inline bool operator==(const vector &c) const{return equals(c);}
	inline bool operator!=(const vector &c) const{return !equals(c);}
