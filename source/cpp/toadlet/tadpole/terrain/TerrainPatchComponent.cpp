...
TerrainPatchComponent::TerrainPatchComponent(Scene *scene):
	mSize(0),
	//mVertexes,
	//mBlocks,
	//mBlockQueue,
...
bool TerrainPatchComponent::setHeightData(scalar *data,int rowPitch,int width,int height,bool water){
	...
	if(water){
		...
		mWaterRenderable=new WaterRenderable(this);
	}
...
scalar TerrainPatchComponent::calculateLayerWeight(int layer,int i,int j,scalar io,scalar jo){
	scalar weights[8];
...
bool TerrainPatchComponent::setLayerData(tbyte *data,int rowPitch,int width,int height){
...
bool TerrainPatchComponent::setMaterial(Material *material){
	mMaterial=material;
...
bool TerrainPatchComponent::stitchToRight(TerrainPatchComponent *terrain,bool restitchDependents){
	TerrainPatchComponent *lt=this;
	TerrainPatchComponent *rt=terrain;
	int y=0;
	...
	VertexBufferAccessor vba(lt->mVertexBuffer,Buffer::Access_BIT_WRITE);
...
void TerrainPatchComponent::updateBlockBoundsRight(Block *block,int q,int x,int y,int s){
	int i,j;
	if(s>1){
...
bool TerrainPatchComponent::unstitchFromRight(TerrainPatchComponent *terrain){
	TerrainPatchComponent *lt=this;
	TerrainPatchComponent *rt=terrain;
	int y;
...
bool TerrainPatchComponent::stitchToBottom(TerrainPatchComponent *terrain,bool restitchDependents){
	TerrainPatchComponent *tt=this;
	TerrainPatchComponent *bt=terrain;
	int x;
	...
	VertexBufferAccessor vba(tt->mVertexBuffer,Buffer::Access_BIT_WRITE);
...
void TerrainPatchComponent::updateBlockBoundsBottom(Block *block,int q,int x,int y,int s){
	int i,j;
	if(s>1){
...
bool TerrainPatchComponent::unstitchFromBottom(TerrainPatchComponent *terrain){
	TerrainPatchComponent *tt=this;
	TerrainPatchComponent *bt=terrain;
	int x;
...
void TerrainPatchComponent::initBlocks(Block *block,int q,int x,int y,int s,bool e){
	int i=0;
	int j=0;
	scalar delta[5];
...
void TerrainPatchComponent::resetBlocks(){
	int i;
	int x,y,s;
...
void TerrainPatchComponent::enableVertex(Vertex *v){
	if(v->enabled==false){
		v->enabled=true;
		if(v->dependent0){
...
void TerrainPatchComponent::disableVertex(Vertex *v){
	if(v->enabled==true){
		v->enabled=false;
		if(v->dependent0){
...
void TerrainPatchComponent::computeDelta(Block *block,const Vector3 &cameraTranslate,float tolerance){
	float fHMin;
