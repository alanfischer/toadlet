...
void ArchiveManager::unmanage(Resource *resource){
	Archive *archive=(Archive*)resource;
	mArchives.erase(stlit::remove(mArchives.begin(),mArchives.end(),archive),mArchives.end());
...
void ArchiveManager::removeDirectory(const String &directory){
	mDirectories.erase(stlit::remove(mDirectories.begin(),mDirectories.end(),cleanPath(directory,true)),mDirectories.end());
}
