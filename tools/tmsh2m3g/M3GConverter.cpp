...
M3GObject3D *M3GConverter::buildSceneGraph(Mesh *toadletMesh,float scale,int forceBytes,bool invertYTexCoord,bool viewable,int activeAnimation){
		...
		for(i=0;i<toadletSkeleton->getNumSequences();++i){
			...
			for(j=0;j<sequence->getNumTracks();++j){
				...
				if(forceBytes==0){
					...
					Log::alert(String("Animation Translation bytes:")+translationBytes);
				}
				...
				if(forceBytes==0){
					...
					Log::alert(String("Animation Rotation bytes:")+rotationBytes);
				}
				if(rotationBytes==1){
	...
	M3GVertexBuffer *vertexes=new M3GVertexBuffer();
	{
		...
		if(positionIndex>=0){
			...
			if(forceBytes==0){
				...
				Log::alert(String("Position bytes:")+bytes);
			}
			if(bytes==1){
		...
		if(texCoordIndex>=0){
			...
			if(forceBytes==0){
				...
				Log::alert(String("TexCoord bytes:")+bytes);
			}
			if(bytes==1){
	...
	for(i=0;i<toadletMesh->getNumSubMeshes();++i){
...
#ifdef USE_ACTC
		{
			...
			Log::alert(String("Number of strips:")+strips.size());
...
#ifdef USE_NVTRISTRIP
		{
			...
			Log::alert(String("Number of strips:")+numGroups);
		...
		if(material!=NULL){
			...
			Log::alert(String("Invalid material on submesh:")+i);
