...
public class Entity{
	...
	public final static int TYPE_PARTICLE=1<<8;
	...
	public void setTranslate(Vector3 translate){
		...
		mTransform.setAt(0,3,mTranslate.x);
		mTransform.setAt(1,3,mTranslate.y);
		mTransform.setAt(2,3,mTranslate.z);
	...
	public static void uint32ColorToFixedColor(int color,int[] fixed){
		fixed[3]=(((color>>24)&0xFF) * Math.ONE) / 255;
		fixed[0]=(((color>>16)&0xFF) * Math.ONE) / 255;
		fixed[1]=(((color>>8)&0xFF) * Math.ONE) / 255;
		fixed[2]=(((color)&0xFF) * Math.ONE) / 255;
	}
	...
	Matrix4x4 mTransform=new Matrix4x4();
	Sphere mWorldBound=new Sphere();
	Matrix4x4 mWorldTransform=new Matrix4x4();
}
