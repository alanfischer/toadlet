...
Blender: 240
Group: 'Export'
Tooltip: 'Save out a Toadlet MSH file'
"""

__author__ = "Andrew Fischer, Lightning Toads Productions, LLC"
__url__ = ["blender", "elysiun", "Lightning Toads Homepage - http://www.lightningtoads.com"]
__version__ = "1.0"

__bpydoc__ = """\
Export meshes to the MSH format for use with the Toadlet Engine.
...
# --------------------------------------------------------------------------
# MSH export v1.0 by Andrew Fischer, Lightning Toads Productions, LLC.
# --------------------------------------------------------------------------
...
def exportObject(filename, object, EXPORT_NORMALS=False, EXPORT_MTL=True, EXPORT_COPY_IMAGES=False):

	print 'Msh Export path: "%s"' % filename
	global MTL_DICT
...
def export(filename):
	...
	# Get USER Options
	options = [\
	('Normals', EXPORT_NORMALS, 'Export vertex normals.'),\
	('Materials', EXPORT_MTL, 'Export materials.'),\
	#('Animation', EXPORT_ANIMATION, 'Export animations.'),\
	('Copy Images', EXPORT_COPY_IMAGES, 'Copy image files to the exported files directory.'),\
	]
	
	if not Draw.PupBlock('Export...', options):
		return
	...
	modelname, extension = splitFilename(filename)
	...
	# Export animations here
	# You have to loop through the frames in the scene to do this
	# Maybe later...

	# Export all selected objects
	for object in objects:
		exportObject(object, EXPORT_NORMALS, EXPORT_MTL, EXPORT_COPY_IMAGES
	
	Window.WaitCursor(0)

Window.FileSelector(export, 'Export Toadlet MSH', newFName('obj'))
