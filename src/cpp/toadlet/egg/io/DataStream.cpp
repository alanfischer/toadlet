...
int DataStream::readBool(bool &b){
	return mStream->read((char*)&b,1);
}

bool DataStream::readBool(){
	bool b=false;
	mStream->read((char*)&b,1);
	return b;
}

int DataStream::readUInt8(uint8 &i){
	return mStream->read((char*)&i,sizeof(i));
}

uint8 DataStream::readUInt8(){
	uint8 i=0;
	mStream->read((char*)&i,sizeof(i));
	return i;
}
...
int DataStream::readBigUInt16(uint16 &i){
	int amt=mStream->read((char*)&i,sizeof(i));
	bigUInt16InPlace(i);
	return amt;
}
...
int DataStream::readBigFloat(float &f){
	int amt=mStream->read((char*)&f,sizeof(f));
	bigFloatInPlace(f);
	return amt;
}
...
float DataStream::readBigFloat(){
	float f=0;
	mStream->read((char*)&f,sizeof(f));
	bigFloatInPlace(f);
	return f;
}

int DataStream::readBigDouble(double &d){
	int amt=mStream->read((char*)&d,sizeof(d));
	bigDoubleInPlace(d);
	return amt;
}
...
int DataStream::writeBool(bool b){
	return mStream->write((char*)&b,1);
}

int DataStream::writeUInt8(uint8 i){
	return mStream->write((char*)&i,sizeof(i));
}
...
int DataStream::writeNullTerminatedString(const String &s){
	return mStream->write(s.c_str(),s.length()+1);
}
...
int DataStream::writeBigFloat(float f){
	bigFloatInPlace(f);
	return mStream->write((char*)&f,sizeof(f));
}

int DataStream::writeBigDouble(double d){
	bigDoubleInPlace(d);
	return mStream->write((char*)&d,sizeof(d));
}
