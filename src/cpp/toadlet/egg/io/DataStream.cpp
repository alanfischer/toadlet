...
int DataStream::readBool(bool &b){
	return mStream->read((byte*)&b,1);
}

bool DataStream::readBool(){
	bool b=false;
	mStream->read((byte*)&b,1);
	return b;
}

int DataStream::readUInt8(uint8 &i){
	return mStream->read((byte*)&i,sizeof(i));
}

uint8 DataStream::readUInt8(){
	byte i=0;
	mStream->read((byte*)&i,sizeof(i));
	return i;
}
...
int DataStream::readNullTerminatedString(String &s){
	...
	int i=0;
	while(true){
		amt+=mStream->read((byte*)string+i,1);
		if(amt==0 || string[i]==0){
			break;
		}
		i++;
...
int DataStream::readBigUInt16(uint16 &i){
	int amt=mStream->read((byte*)&i,sizeof(i));
	bigUInt16InPlace(i);
	return amt;
}
...
int DataStream::readBigFloat(float &f){
	int amt=mStream->read((byte*)&f,sizeof(f));
	bigFloatInPlace(f);
	return amt;
}
...
float DataStream::readBigFloat(){
	float f=0;
	mStream->read((byte*)&f,sizeof(f));
	bigFloatInPlace(f);
	return f;
}

int DataStream::readBigDouble(double &d){
	int amt=mStream->read((byte*)&d,sizeof(d));
	bigDoubleInPlace(d);
	return amt;
}
...
int DataStream::readBigInt16String(String &s){
	...
	amt+=mStream->read((byte*)string,len);
	string[len]=0;
	s=string;
	delete[] string;
	return amt;
...
int DataStream::writeBool(bool b){
	return mStream->write((byte*)&b,1);
}

int DataStream::writeUInt8(uint8 i){
	return mStream->write((byte*)&i,sizeof(i));
}
...
int DataStream::writeNullTerminatedString(const String &s){
	return mStream->write((byte*)s.c_str(),s.length()+1);
}
...
int DataStream::writeBigFloat(float f){
	bigFloatInPlace(f);
	return mStream->write((byte*)&f,sizeof(f));
}

int DataStream::writeBigDouble(double d){
	bigDoubleInPlace(d);
	return mStream->write((byte*)&d,sizeof(d));
}
...
int DataStream::writeBigInt16String(const String &s){
	...
	amt+=mStream->write((byte*)s.c_str(),len);
	return amt;
}
