...
TOADLET_C_API MotionDevice *new_HTCMotionDevice(){
	return new HTCMotionDevice();
}
...
HTCMotionDevice::HTCMotionDevice():FilteredMotionDevice(),
	...
	mPollThread(NULL)
{
	...
	Logger::alert(Categories::TOADLET_FLICK,
		String("HTCMotionDevice:")+(mSensor!=NULL));
}

HTCMotionDevice::~HTCMotionDevice(){
...
bool HTCMotionDevice::available(){
	return mSensor!=0;
}

bool HTCMotionDevice::create(){
...
void HTCMotionDevice::setPollSleep(int ms){
	mPollSleep=ms;
}

void HTCMotionDevice::setFilterAlpha(real alpha){
	mAlpha=alpha;
}

void HTCMotionDevice::setNativeOrientation(bool native){
	mNative=native;
}

void HTCMotionDevice::setListener(MotionDeviceListener *listener){
	mOuterMutex.lock();
		mMotionData.time=0;
...
MotionDevice::State HTCMotionDevice::getState(){
	return mState;
}

void HTCMotionDevice::run(){
