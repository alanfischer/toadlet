...
Logger::alert("TCPConnection notifying disconencted");
		mConnectionListener->disconnected(this);
		mConnectionListener=NULL;
	}
}
...
int TCPConnection::send(const byte *data,int length){
	...
	TOADLET_TRY
		if((amount=mSocket->send(mOutPacket->getOriginalDataPointer(),mOutPacket->length()))<0)
...
Logger::alert(String("AMT:")+amount);
	if(amount<0 && opened()){
		Logger::alert("Amount<0 && opened!");
		close();
	}
