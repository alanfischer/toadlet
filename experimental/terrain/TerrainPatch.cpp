...
void TerrainPatch::rendererRegistered(Renderer *r){
	// TODO,MULTIPIPE: Flag certain index buffers as specific to only one rendering context

	RendererData data;

	data.indexBuffer=mEngine->getIndexBufferManager()->load(new IndexBuffer());
	data.indexBuffer->setStatic(false);

	data.detailBuffer=mEngine->getIndexBufferManager()->load(new IndexBuffer());
	data.detailBuffer->setStatic(false);

	mRendererData[r]=data;
}

void TerrainPatch::rendererUnregistered(Renderer *r){
	mRendererData.erase(mRendererData.find(r));
}
